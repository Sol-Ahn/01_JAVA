...
public class C_Casting {
      ...
      *  
      *  --> 형변환을 이용하면 위 규칙을 깰 수 있다. --> 그러나 결국 형변환은 위의 규칙을 맞춰나가는 작업
      * 형변환의 종류
      * 1. 자동 형변환
      * 2. 강제(명시적) 형변환
      * 
	...
	public void autoCasting() {
		...
		int i2 = 2147483647;
		long l2 = i2 + 1L; // 컴파일러가 2147483647L + 1L 형태로 변환해서 계산을 진행함. 이것도 자동 형변환
		...
		// 2진수 무한소수를 정확히 표현하는데 한계가 있다.
		// 그래서 오차가 발생한다.
		
		// char(2byte) -> int(4byte)
		int i4 = 'A'; //int형 변수에 char형 리터럴 값을 대입
		/* - char -> int 자동 형변환이 가능한 이유
		 * char라는 자료형 변수는 문자 자체를 저장하는 변수가 아닌 
		 * 2byte(0~65565) 범위 내의 숫자를 저장하고,
		 * 유니코드라는 문자표에서 해당 번호와 일치하는 문자를 찾아
		 * 출력하는 자료형이다.
		...
		// 초기화된 이후 byte, short, char 자료형 변수의 
		// 값을 이용하여 계산(연산) 진행 시 
		// 무조건 int형으로 연산처리가 진행된다.
		
		// byte 변수끼리 연산시 시
		// 변수의 값이 int형으로 처리되기 때문에 
		// bResult에 값 저장이 불가능한 상태이다.
		// --> int형 결과를 byte로 강제 형변환하면 저장 가능!
		byte bResult = (byte)(bNum1 + bNum2); // 강제 형변환
		...
		public void forcedCasting() {
			...
			// 에러 해결방법
			// 방법 1) 사용자가 타협을 본다.
			// result의 타입을 long형으로 변환
			long result = iNum1 + lNum1;
			
			// 방법 2) iNum1 + lNum1의 연산 결과를 
			// int형으로 강제 형변환한다.
			int result2 = (int)(iNum1 + lNum1);
		
			// 방법 3) lNum1을 int형으로 강제 형변환한다.
