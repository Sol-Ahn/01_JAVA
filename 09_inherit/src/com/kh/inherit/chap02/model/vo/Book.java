...
public class Book extends Object{
	// extends Object 미작성 시 JVM에 의해서 컴파일 단계에서 자동 추가
...
//	}
	...
	/* @Override 어노테이션
	 * - 어노테이션(Annotation) : 컴파일러에게 특정 코드나 상황을 알려주는 역할
	 * 
	 * - @Override : 컴파일러에게 해당 부모 메소드가 자식에 의해 오버라이딩(재정의)이 되었음을 알려줌.
	 * 
	 * + 과거에는 @Override 표시가 필수였으나, 현재는 없어도 가능함.
	 *  대신 개발자들간의 코드 공유 시 가독성을 위하여 필히 작성하는 것이 좋음.
	 * 
	 * */
	...
	public String toString() {
		// Object.toString() 용도
		// 생성된 객체의 필드 정보를 모두 출력하는 메소드를 따로 만들지 말고, 
		// 이를 오버라이딩해서 사용하도록 Java에서 제공하는 메소드
	...
	// 오버라이딩 후 equals() : 객체가 가지고 있는 필드값이 같은지를 판별
	...
	public boolean equals(Object obj) {
		...
		// 3. 두 객체간의 필드값 비교 시작
		if(this.title != null && other.title != null) { // 제목 비교
			// 두 Book객체에 모두 title이 있을 경우
		...
		if(this.author != null && other.author != null) { // 저자 비교
		...
		if(this.price != other.price) { // 가격 비교
			// price는 기본자료형이기 때문에 
	...
	*/
	...
	@Override
	public int hashCode() { 
		// 중복되지 않는 hashCode를 만드는데 왜 '31'을 사용하는가?
